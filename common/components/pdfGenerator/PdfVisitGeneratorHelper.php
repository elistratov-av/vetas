...
class PdfVisitGeneratorHelper
{
    ...
    {
        //        $file = '../common/components/pdfGenerator/PdfVisitGeneratorResult.txt';
//        file_put_contents($file, print_r($visit, true).PHP_EOL, FILE_APPEND);
...
//        }
        ...
        if ($visit->owner->is_veteran_infosoc) {
            $privileges[] = 'Ветеран ВОВ';
        }

        if ($visit->owner->is_disabled_infosoc) {
            $privileges[] = 'Инвалид I группы';
        }

        if ($visit->owner->is_blind_infosoc) {
            $privileges[] = 'Слабовидящий с животным поводырем';
        }

        if ($visit->owner->is_family_disabled_children_infosoc) {
            $privileges[] = 'Семьи, воспитывающие детей-инвалидов в возрасте до 23 лет';
        }
        if (count($privileges) == 0) {
            $privileges[] = 'отсутствует';
        }
...
SQL;
    ...
    public static function getSerialServiceId(int $visitId, int $reportId): array
    {
        $visitServiceId = (new Query())
            ->select(['vgs.id',  'gs.id_service_type', 'vgs.id_pet', new Expression("STRING_AGG(pi.identification_code, ', ') as chips")])
            ...
            ->leftJoin(PetIdentification::tableName() . ' pi', 'pi.id_pet = vgs.id_pet AND pi.id_ident_type = 1')
            ->where([
                'AND',
                ['gsr.id_report' => $reportId],
                ['vgs.id_visit' => $visitId],
            ])
            ->groupBy('vgs.id, gs.id_service_type, vgs.id_pet')
            ->all();
