...
use app\common\components\reports\handlers\PetHotelStayReport\v1\builders\PrintExcelBuilder;
        ...
        switch ($fileType) {
            case FileTypeDefinition::EXCEL:
                return new PrintExcelBuilder(
                    $this,
                    new FileOptionsXlsDto(['name' => 'pet-hotel-stay-report_'. date('Ymd_His') . '.xls']),
                    ...$printBuilderConfig
                );
            default:
                throw new \Exception('Отчета не потдерживает создание отчета в указаном формате');
        }
