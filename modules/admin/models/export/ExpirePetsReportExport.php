...
class ExpirePetsReportExport
{
        ...
        foreach ($this->report as $report_row) { // идем поэлементно по массиву с результатами запроса
            ...
            if($currentOrgId !== $report_row['id_reg_organization']) {
                ...
                $this->_renderHeaderRow(
                    $report_row['short_name'] ?? 'Организация не указана',
                    self::STYLE_ORGGREY
                );
            }
    ...
    protected function _renderRow($title, $report_row, $style = self::STYLE_INFOROW)
    {
        ...
        $sheet->fromArray(
            [
                ...
                $report_row['reg_expire_date'],
                $report_row['reason_name']
            ],
            NULL,
    ...
    protected function renderHead($from, $to)
    {
        ...
        $message = 'Отчет о снятии с учета с ' . $from . ' по ' . $to;
        $sheet->setCellValueByColumnAndRow(
            '1',
            $this->currentRowNum,
            $message
        ...
        $sheet
            ->setCellValue('G2', 'Дата снятия с учета');

        $sheet
            ->setCellValue('H2', 'Причина снятия с учета');
