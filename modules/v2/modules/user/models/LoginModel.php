...
class LoginModel extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['login', 'password'], 'trim'],
            [['login', 'password'], 'string', 'skipOnEmpty' => false],
        ];
    }
    ...
    public function login()
    {
        ...
        $this->user = UserModel::findByLogin($this->login);
    ...
    private function checkUser($action)
    {
        ...
        $attribute = 'user';
        if ($this->user === null) {
            $this->addError('password', 'Введен неверный логин или пароль');

            return;
        }
        if ($this->user->is_blocked) {
            $this->addError('password', 'Ваша учетная запись заблокирована');

            return;
        }
        if ($this->user->isBlockedUntil()) {
            $this->addError('password', 'Ваша учетная запись заблокирована до ' . \DateTime::createFromFormat('Y-m-d H:i:s', $this->user->block_until)->format('H:i:s d.m.Y'));

            return;
        }

        if ($action == 'login' && !$this->user->validatePassword($this->password)) {
            $this->addError('password', 'Введен неверный логин или пароль');
            $this->user->addLoginAttempt();
            if (!$this->user->checkLoginAttempts()) {
                $this->user->blockUntil();
                $this->addError($attribute, 'Ваша учетная запись заблокирована на 30 минут');
            }
