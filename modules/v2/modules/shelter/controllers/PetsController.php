...
class PetsController extends BaseController
{
    ...
    )
    {
        ...
        */
        ...
        $subQuery = ShelterGuests::find()
            ...
            ->andWhere('ph.date >= sg.arrival_date');
            ...
            if (($model = Pets::findOne($data['id'])) && $model->setAttributes($data)) {
                ...
                if ($model->save()) {
                    ...
                    if ($data['identification_1']) {
                        ...
                        $identification->setAttributes([
                            'id_ident_type' => IdentificationTypes::findIdentificationTypeId('метка'),
                    ...
                    if($identification->save())
                    {
                       ...
                       return $this->errorResponse($identification);
                    }
                }
            }
        }
    ...
    private function getHistory($pet_id, $is_animal) {
        ...
        ->where(['id_pet' => $pet_id, ])        
        ->andWhere(['in', 'event', 
            [PetHistory::HISTORY_EVENT_REGISTERED, 
            PetHistory::HISTORY_EVENT_SHELTER, 
            PetHistory::HISTORY_EVENT_CASTRATED,
            PetHistory::HISTORY_EVENT_VACCINATION,
            PetHistory::HISTORY_EVENT_QUARANTINE,
            PetHistory::HISTORY_EVENT_QUARANTINE_END,
            PetHistory::HISTORY_EVENT_DEPARTURE,
            PetHistory::HISTORY_EVENT_DEREGISTERED]])
        ->with('organization')
        ->with('creater')
        ...
        if ($models) {
            ...
            foreach ($models as &$model) {
                ...
                if ($model['event'] === PetHistory::HISTORY_EVENT_AVIARY && $aviary = Aviary::findOne((int)$model['options'])) {
                    ...
                    $options_title = '';
                    if($model['options_str']) {
                        $options_title = $model['options_str'];
                    } else {
                        $departure_reason = $record->departure_reason;
                        if($departure_reason){
                            if($departure_reason == 'ESCAPE'){
                                $options_title = 'Побег';
                            }else if($departure_reason == 'DEATH'){
                                $options_title = 'Падёж';
                            }else if($departure_reason == 'EUTHANASIA'){
                                $options_title = 'Эвтаназия';
                            }else if($departure_reason == 'RETURNED_TO_NEW_OWNER'){
                                $options_title = 'Передача новому владельцу';
                            }else if($departure_reason == 'RETURNED_TO_OWNER'){
                    ...
                    if($model['options']) {
                        $reason = RegExpireReasons::findOne(['id' => (int)$model['options']]); 
                    } else {
    ...
    public function actionAnimalHistory($pet_id) {
        return $this->getHistory($pet_id, true);
    }

    public function actionHistory($pet_id)
    {
        return $this->getHistory($pet_id, false);
    }
    ...
    )
    {
        ...
        if (($result = $model->departurePet($id_pet, $departure_date, $status, $departure_comment, $departure_specialist, $id_owner, $departure_documents))
            ...
            if($status == 'ESCAPE'){
                ...
                $options_title = 'Падёж';
            }else if($status == 'EUTHANASIA'){
                $options_title = 'Эвтаназия';
            }else if($status == 'RETURNED_TO_NEW_OWNER'){
                $options_title = 'Передача новому владельцу';
            }else if($status == 'RETURNED_TO_OWNER'){
                 $petowner = PetOwners::findOne(['id' => $id_owner]);
                if($petowner->fullname){
                    $options_title = 'Возврат прежнему владельцу '.$petowner->fullname;
                }
            }
            if($departure_comment){
                $options_title=$options_title." (".$departure_comment.")";
            }
            $pet_history_model->options_str = $options_title;
    ...
    public function actionUpdateStatus(int $id_pet, string $status)
    {
        ...
        $pet->setAttribute('prev_status', $pet->status);
