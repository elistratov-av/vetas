 ...
 *
 */
class TimesheetsSave extends Model
{
    /**
     * @var array
     */
    ...
    protected function internalSave()
    {
        ...
        // удаляем старые
        $this->deleteOldTimesheets();

        // сохраняем новые (по спецам пачками)
        foreach ($this->formatted_timesheets as $specialist_id => $timesheets) {
            /*
             * Родительские (и верхоуровневые)
             * (нам надо собрать массив id для проставления parent_id в дочерних)
             */
            $parent_result = $this->saveParentTimesheets(
                $timesheets['parents']
            );
            if ($parent_result !== true || $this->hasErrors()) {
                Timesheets::getDb()->transaction->rollBack();
                return false;
            }
             ...
             * Дочерние
             */
            $childs_result = $this->saveChildsTimesheets(
                $timesheets['childs']
            );

            if ($childs_result !== true || $this->hasErrors()) {
                Timesheets::getDb()->transaction->rollBack();
                return false;
            }
        }
...
//        \Yii::debug($visits_to_transfer, 'timesheet_visits_to_transfer');
     ...
     * @return int
     */
    protected function markTransferVisits($visits_list)
    {
        ...
        if ($result) {
            ...
            foreach ($specialists as $specialist) {
                foreach ($visits_list as $id_visit) {
    ...
    /**
     * Сохраняем родительские таймшиты,
     * для них надо получать и сохранять их id что бы потом проставлять parent_id в дочерних
     * Заодно проверям, что не пересекаемся с таймшитами в БД
     * В текущей реализации вызывается со списком от каждого специалиста отдельно
     *
     * @param $parent_formatted_timesheets
     *
     * @return bool Удачно/неудачно
     * @throws \yii\db\Exception
     */
    protected function saveParentTimesheets($parent_formatted_timesheets): bool
    {
        // Такой кейс возможен когда "очищают" день
        if (empty($parent_formatted_timesheets)) {
            return true;
        }

        $shiftTypeInvalidIntersections = ShiftTypeInvalidIntersections::find()
            ->select(['array_to_string(array_agg(id_type), \',\')'])
            ->scalar();

        foreach ($parent_formatted_timesheets as $timesheet) {
            $from = $timesheet['date_from']->format(self::TIMESHEET_DATE_FORMAT);
            $to = $timesheet['date_to']->format(self::TIMESHEET_DATE_FORMAT);

            /*
             * Проверяем, что в бд нет пересекающихся с ним таймшитов
             */
            $exist = Timesheets::find()
                ->leftJoin('shifts', 'shifts.id = timesheets.id_shift')
                ->where([
                    'AND',
                    ['id_specialist' => $timesheet['id_specialist']],
                    new Expression("date && tsrange(:from, :to, '[)')", [
                        ':from' => $from,
                        ':to' => $to,
                    ])
                ])
                ->andWhere([
                    'AND',
                    ['in', 'shifts.id_type', explode(',', $shiftTypeInvalidIntersections)],
                    $timesheet['id_shift_type'] . ' IN ('.$shiftTypeInvalidIntersections.')',
                ])
                ->exists();

            if ($exist) {
                $this->addError(
                    'timesheet', 'Существует родительский таймшит пересекающийся по времени с переданным'
                );
                return false;
            }

            foreach ($timesheet['times'] as $num => $times) {
                if (empty($times['from_time']) || empty($times['to_time'])) {
                    unset($timesheet['times'][$num]);
                }
            }

            // Так как используются insert/batchInsert, behaviors не работают
            $now = date('Y-m-d H:i:s');
            $id_user = $this->blameableId();

            /*
             * Добавляем и запоминаем маппинг
             */
            Timesheets::getDb()->createCommand()->insert(Timesheets::tableName(),
                [
                    'id_specialist' => $timesheet['id_specialist'],
                    'vaccination_station_id' => $timesheet['vaccination_station_id'] ?? null,
                    'id_shift' => $timesheet['id_shift'],
                    'date' => '["' . $from . '","' . $to . '")',
                    'created_by' => $id_user,
                    'updated_by' => $id_user,
                    'created_at' => $now,
                    'updated_at' => $now,
                    'times' => json_encode($timesheet['times']),
                    'cabinet_type' => $timesheet['cabinet_type'],
                ])->execute();

            $this->map_patent_temp_uid_to_db_id[$timesheet['temp_uid']] = Timesheets::getDb()->getLastInsertID();
        }
    ...
    /**
     * Сохраняем дочерние таймшиты,
     * проставляя при этом parent_id
     * В текущей реализации вызывается со списком от каждого специалиста отдельно
     *
     ...
     * @throws \yii\db\Exception
     */
    protected function saveChildsTimesheets($child_formatted_timesheets)
    {
        // Такой кейс возможен когда "очищают" день
        if (empty($child_formatted_timesheets)) {
            return true;
        }
        ...
        // собираем значимые поля в массив-массивов для сохранения пачкой
        $for_save = [];
        foreach ($child_formatted_timesheets as $timesheet) {
            $parent_id = $this->getParentIdForChildTimesheet(
                $timesheet['temp_uid']
            );

            if (empty($parent_id)) {
                $this->addError('timesheet', 'Не удалось связать дочерние таймшиты с родительскими');
                return false;
            }
        ...
        Timesheets::getDb()
            ->createCommand()
            ->batchInsert(
                Timesheets::tableName(),
                // следить за последовательностью при редактировании этой части!
                [
                    'id_specialist',
                    'vaccination_station_id',
                    'id_shift',
    ...
    protected function validateAndFormatTimesheets()
    {
        ...
        $dTimesheet = [];
        foreach($this->timesheet as  $timesheet) {
            $shift = Shifts::find()->where(['id' => $timesheet['id_shift']])->one();
            if (!empty($shift) && $shift->daily == true) {
                if (count($this->timesheet) > 1) {
                    if (date('H:i:s', strtotime($timesheet['from'])) == '00:00:00' && date('Y-m-d', strtotime($timesheet['from'])) == date('Y-m-d', strtotime($timesheet['to']))) {
                        $timesheet['daily_to'] = true;
                        $dTimesheet[] = $timesheet;
                    }else{
                        $testTimeSheetTo = Timesheets::find()
                            ->where(['id_specialist' => $timesheet['id_specialist']])
                            ->andWhere(['id_shift' => $shift->id])
                            ->andWhere(['date' => '["' . date('Y-m-d', strtotime($timesheet['from'])) . ' 00:00:00","' . $timesheet['from'] . '")'])
                            ->one();

                        $tempTimesheet = $timesheet;
                        if (empty($testTimeSheetTo)) {
                            $tempTimesheet['to'] = date('Y-m-d 23:59:59', strtotime($tempTimesheet['from']));
                            $tempTimesheet['daily_from'] = true;
                            $tempTimesheet['daily_to'] = false;
                        }else{
                            $tempTimesheet['to'] = $tempTimesheet['from'];
                            $tempTimesheet['from'] = date('Y-m-d 00:00:00', strtotime($tempTimesheet['from']));
                            $tempTimesheet['daily_from'] = false;
                            $tempTimesheet['daily_to'] = true;
                        }

                        $dTimesheet[] = $tempTimesheet;

                        $testTimeSheetFrom = Timesheets::find()
                            ->where(['id_specialist' => $timesheet['id_specialist']])
                            ->andWhere(['id_shift' => $shift->id])
                            ->andWhere(['date' => '["' . $timesheet['from'] . '","' . date('Y-m-d', strtotime($timesheet['from'])) . ' 23:59:59")'])
                            ->one();

                        if (empty($testTimeSheetFrom) && empty($testTimeSheetTo)) {
                            $tempTimesheet = $timesheet;
                            $tempTimesheet['from'] =  date('Y-m-d 00:00:00', strtotime($tempTimesheet['to']));
                            $tempTimesheet['daily_from'] = false;
                            $tempTimesheet['daily_to'] = true;
                            $dTimesheet[] = $tempTimesheet;
                        }
                ...
                $timesheet['daily_from'] = false;
                $timesheet['daily_to'] = false;
                $dTimesheet[] = $timesheet;
            }
        }
        
        $this->timesheet = $dTimesheet;
        ...
        foreach ($timesheets_by_specialist as $id_specialist => &$timesheets) {
            ...
            foreach ($timesheets as $timesheet_key => &$timesheet) {
                ...
                if ($timesheet['daily_from'] == false && $timesheet['daily_to'] == false) {
    ...
    protected function validateRootLevelShiftsIntersection($root_level_formatted_timesheets)
    {
        ...
        foreach ($root_level_formatted_timesheets as $next) {
            ...
            if ($next['date_from'] < $prev['date_to'] &&
                $this->existShiftTypeInvalidIntersections($next['id_shift_type'], $prev['id_shift_type'])
            ) {
                $this->addError(
                    'timesheet',
    ...
    /**
     * @param int $nextIdShiftType
     * @param int $prevIdShiftType
     * @return bool
     */
    protected function existShiftTypeInvalidIntersections(int $nextIdShiftType, int $prevIdShiftType): bool
    {
        $shiftTypeInvalidIntersections = ShiftTypeInvalidIntersections::find()
            ->select(['array_to_string(array_agg(id_type), \',\')'])
            ->scalar();
        ...
        if (empty($shiftTypeInvalidIntersections) === false) {
            return in_array($nextIdShiftType, explode(',', $shiftTypeInvalidIntersections)) &&
            in_array($prevIdShiftType, explode(',', $shiftTypeInvalidIntersections));
        }
    ...
    /**
     * Очищаем укзанное пространство специалисты-время
     * СМ ПРИМЕЧАНИЕ К КЛАССУ - удаление идет в диапазоне (_date_from ; _date_to_for_clear_old)
     *
     * @throws \yii\db\Exception
     */
    protected function deleteOldTimesheets(): void
    {
        $specs = implode(', ', $this->specialists);
        $params = [
        ...
        /**
         * Очистим ВСЕ старые данные таймшитов из диапозона, переданного для записи для ВСЕХ переданных врачей
         */
        $condition = "id_specialist IN ({$specs}) AND date && tsrange(:date_from, :date_to, '()')";
        $sqlGetTimesheetIdsForDelete = <<<SQL
...
SQL;
        ...
        $idsForDelete = Timesheets::getDb()
            ->createCommand($sqlGetTimesheetIdsForDelete, $params)
            ->queryColumn();

        Timesheets::deleteAll(['IN', 'id', $idsForDelete]);
    }
     ...
     * @return bool|mixed
     */
    protected function getParentIdForChildTimesheet($child_timesheet_temp_uid)
    {
        if (empty($child_timesheet_temp_uid) || !array_key_exists($child_timesheet_temp_uid,
                $this->map_child_to_parent)) {
            return false;
        }

        $parent_temp_uid = $this->map_child_to_parent[$child_timesheet_temp_uid];

        if (empty($parent_temp_uid) ||
            !array_key_exists($parent_temp_uid, $this->map_patent_temp_uid_to_db_id) ||
            empty($this->map_patent_temp_uid_to_db_id[$parent_temp_uid])
        ) {
            return false;
        }

        return $this->map_patent_temp_uid_to_db_id[$parent_temp_uid];
    }
