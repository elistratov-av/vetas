...
use app\modules\adminv\models\export\BalanceExpReportExport;
use yii\data\ActiveDataProvider;
use yii\db\Expression;
use yii\db\Query;

/**
 * Отчет по использованию расходников
 *
 * Class BalanceTmcReportController
 * @package app\modules\adminv\controllers\statistics
 */
class BalanceExpReportController extends StaticticsController
{
    /**
     * @param string $from
     * @param string $to
    ...
    private function buildQuery($from, $to, $organizations)
    {
        ...
        $fromQuery = BalanceFlow::find()
            ...
            ->andWhere(['b.type_tmc' => 'exp_material'])
            ->groupBy([
                'b.id_organization',
                'o.id',
                'o.short_name',
...
//        $organizations = $this->organizationsOptions();
        $exportedReport = new BalanceExpReportExport();
        $exportedReport->export($mainQuery, "Отчет по использованию расходных материалов c {$this->from} по {$this->to}.xlsx", $this->from, $this->to);
    }
}
