...
use app\modules\adminv\models\export\BalanceVaccReportExport;
use yii\db\Query;
...
/**
 * Отчет по использованию вакцин
 *
 * Class BalanceTmcReportController
 * @package app\modules\adminv\controllers\statistics
 */
class BalanceVaccReportController extends StaticticsController
{
    /**
     * @param string $from
     * @param string $to
        ...
        $fromQuery = BalanceFlow::find()
            ...
            ->select([
             ...
             and (ba.from_id_organization != ba.to_id_organization or ba.from_id_organization is null)) or bf.id_balance_action isnull) and bf.created_at::date between :from and :to) then bf.count else 0 end) as income', [':from' => $from, ':to' => $to]),
                ...
                new Expression('sum(case when bf.created_at::date > :to and (ba.status = \'C\' or (bf.id_visit_service notnull and v.channel not in (44, 45, 46)) or (bf.id_balance_action isnull)) then (case
    ...
    public function actionExport()
    {
        ...
        $exportedReport = new BalanceVaccReportExport();
        $exportedReport->export($mainQuery, "Баланс вакцин c {$this->from} по {$this->to}.xlsx", $this->from, $this->to);
