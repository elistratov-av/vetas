 ...
 * @package app\modules\soap\queue
 */
class ETPChangeStatusJob extends BaseObject implements JobInterface
{
     ...
     */
    public $status_id;
    private $log_category = 'soap_queue';
    ...
    {
        // Временный хак, чтобы отлжить выполнение, т.к. не все данные успевают сохраниться до выполнения очереди
        sleep(5);
        try {
            ...
            foreach ($this->etp_status as $etp_status) {
                $status = ETPHelper::getStatus($etp_status);
                ...
                \Yii::info("{$sender->getServiceNumber()}|{$this->visit_id}|{$code}|", $this->log_category);
