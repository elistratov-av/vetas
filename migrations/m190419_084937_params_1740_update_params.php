...
/**
 * Class m190419_084937_params_1740_update_params
 */
class m190419_084937_params_1740_update_params extends Migration
{
    /**
     * {@inheritdoc}
     */
    public function safeUp()
    {
        ...
        foreach ($items as $item) {
            // tech_name;name;datatype;datatype_details;visit_flag;№
            $tech_name = trim($item[0]);
            $name = trim($item[1]);
            $datatype = strtolower(trim($item[2]));
            $datatype_details = ($datatype == 'dict') ? strtolower(trim($item[3])) : trim($item[3]);
            $visit_flag = ($item[4] == '') ? false : ($item[4] === 'true' ? true : false);

            $columns = compact('tech_name', 'name', 'datatype', 'datatype_details', 'visit_flag');
            ...
            if ($param === null) {
                $paramsNotFound++;
                Console::output(Console::ansiFormat('Creating param [' . $tech_name . ']...', [Console::FG_YELLOW]));
            ...
            Console::output(Console::ansiFormat('Updating param [' . $tech_name . '] (' . implode(', ', $columns_to_update) . ')...', [Console::FG_YELLOW]));
            $param->load($to_update, '');
            if ($param->save(true, array_merge($columns_to_update, ['updated_at']))) {
                $paramsUpdated++;
                Console::output(Console::ansiFormat('   - OK', [Console::FG_GREEN]));
            } else {
                $paramsUpdateErrors++;
        ...
        if (!empty($techNames)) {
            $paramsToRemove = $this->findParamsByTechNames($techNames, true);
            if (!empty($paramsToRemove)) {
                foreach ($paramsToRemove as $param) {
                    Console::output(Console::ansiFormat('Removing param [' . $param->tech_name . ']...', [Console::FG_YELLOW]));
                    if (!$param->delete()) {
                        $paramsRemoveErrors++;
                        Console::output(Console::ansiFormat('   - Error', [Console::FG_RED]));
                    } else {
                        ...
                        // check gov_services_params
                        $this->db
                            ->createCommand()
                            ->delete(GovServicesParams::tableName(), ['id_param' => $param->id])
                            ->execute();
                        // check reports_params
                        $this->db
                            ->createCommand()
                            ->delete(ReportsParams::tableName(), ['id_param' => $param->id])
                            ->execute();
                        // check visit_param_values
                        $this->db
                            ->createCommand()
                            ->delete(VisitParamValues::tableName(), ['id_param' => $param->id])
                            ->execute();
                        // check visit_service_param_values
                        $this->db
                            ->createCommand()
        ...
        Console::output(Console::ansiFormat('Created params:' . $paramsCreated, [Console::FG_GREEN]));
        if (!empty($paramsCreateErrors)) {
            Console::output(Console::ansiFormat('Create errors:' . $paramsCreateErrors, [Console::FG_RED]));
        }
        Console::output(Console::ansiFormat('Updated params:' . $paramsUpdated, [Console::FG_GREEN]));
        ...
        Console::output(Console::ansiFormat('Removed params:' . $paramsRemoved, [Console::FG_GREEN]));
    ...
    /**
     * @param array $tech_names
     * @param bool $inverse
     * @return \app\models\db\Params[]
     */
    private function findParamsByTechNames($tech_names, $inverse = false)
    {
        $condition = ($inverse === true)
            ? ['not in', 'tech_name', $tech_names]
            : ['in', 'tech_name', $tech_names];
...
P0_Petweight;Вес животного (кг);numeric;4,3;false;64
...
P36_Amilazavalue;ά- амилаза - результат исследований;text;15;false;328
P37_Amilazadesc;ά- амилаза - примечание;text;255;false;329
...
P2_SerialServiceNum;Экспертиза (исследование) №;text;50;false;501
CSV;
