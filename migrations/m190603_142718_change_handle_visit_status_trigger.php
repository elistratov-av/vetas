...
/**
 * Class m190603_142718_change_handle_visit_status_trigger
 */
class m190603_142718_change_handle_visit_status_trigger extends Migration
{
    /**
     * {@inheritdoc}
     */
...
BEGIN
    ...
    */
    IF OLD.status != NEW.status AND (EXISTS (SELECT 1 FROM etp.message WHERE visit_id = NEW.id)) THEN
        CASE
            WHEN NEW.status = 'C' AND OLD.status != 'C1' THEN -- проверка OLD.status != 'C1' для того чтобы не генерировать повторное событие в очереди
