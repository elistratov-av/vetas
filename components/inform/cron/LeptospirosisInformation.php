...
/**
 * Class LeptospirosisInformation
 * @package app\common\components\inform\cron
 */
class LeptospirosisInformation extends Information
{
    /**
     * @return string
     */
    ...
    {
        return RemindVaccinationLeptoEvent::EVENT_CODE;
    }
     ...
     * @return RemindVaccinationLeptoEvent|SubscriptionEvent
     */
    public function getEvent($contacts, Pets $pet, PetOwners $owner)
    {
        return new RemindVaccinationLeptoEvent([
            'contacts' => $contacts,
            'owner' => $owner,
            'pet' => $pet,
            'id_pet'=> $pet->id
    ...
    public function getPetsForInformation()
    {
        ...
        $result = Pets::find()
            ->forInformation(25)
            ->withLeptospirosisVaccination()
            ->withInformationLog($this->getType())
            ->andWhere(new Expression("(information_log.pet_id IS NULL OR information_log.date < leptospirosis_vaccination.inform_date)"))
            ->each($this->batchSize);
