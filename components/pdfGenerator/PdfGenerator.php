...
class PdfGenerator extends ResourceFileRepository
{
    ...
    /** Шаблон приема */
    public const VISIT_DESCRIPTIONS_TEMPLATE = 'visit_descriptions';
            ...
            foreach ($reports as $report) {
                ...
                if ($report === self::VSD_REPORT) {
                    ...
                    foreach ($id_services as $id_service) {
                        //Костыль для предотращения повторной генирации отчета по той же услуге
                        if(in_array($id_service['id'], $idServiceIdUsed)){
                            continue;
                        }
                        $idServiceIdUsed[] = $id_service['id'];

                        $reportParamsValues = $this->findParamsValues($id_service, 'files');
                        $datas = $this->handler->prepareOutput($reportParamsValues, true);
                        
                        foreach ($datas['params'] as $datates) {
                            $html_contents[] = [
                                'format' => $format,
                                'content' => '<sethtmlpageheader value="off"/><pagebreak resetpagenum=1/>' .
                                    $this->getHtmlContent($datas['id_report'], $datates)];
                        }
                    }
                    $paramsCssInline .= $this->getCss($datas['id_report']);
                }
            }
        }
